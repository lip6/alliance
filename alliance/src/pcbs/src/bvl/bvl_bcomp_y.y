...
/* Modif OLIVIER : le type etait force a 'namealloc ("natural")' dans   */
/* tous les cas                                                         */
		  if ($4.VALU == NATURAL)
		  	type = namealloc ("natural");
		  else
		  	type = namealloc ("string");
...
generic_expression
	...
	| StringLit  /* Modif OLIVIER */
		{
		char *s;

		s = namealloc($1);
		BVL_INTLST = addchain (BVL_INTLST,(void *)s);
		}
	| generic_aggregate
	;
...
/* Modif OLIVIER 
   was :if ((type != NATURAL) || (mode != 0) || (kind != 0))
*/
        if (((type != NATURAL && type != STRING)) || (mode != 0) || (kind != 0))
          {
          errflg = 1;
          bvl_error (77,NULL);
          }
