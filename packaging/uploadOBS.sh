 ...
 if [ "${doSources}" = "true" ]; then
   ...
   ./packaging/git-archive-all.sh -v --prefix alliance-${version}/ --format tar.gz alliance-${version}.tar.gz
   #git archive --prefix=alliance-${version}/ --format=tar.gz -o alliance-${version}.tar.gz HEAD
 fi

 echo "* Update files in OBS project directory."
 echo "  OBS package directory: \"${obsDir}\"."
 for distribFile in packaging/alliance.spec                \
                    packaging/alliance-rpmlintrc           \
                    packaging/alliance.dsc                 \
                    packaging/debian.copyright             \
                    packaging/debian.changelog             \
                    packaging/debian.control               \
                    packaging/debian.rules                 \
                    packaging/debian.alliance.install      \
                    packaging/debian.alliance-libs.install \
                    packaging/debian.alliance-doc.install  \
                    alliance-${version}.tar.gz             \
		    ; do
